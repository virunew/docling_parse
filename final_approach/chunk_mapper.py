...
def map_chunks_to_spec(input_chunks_file: str, output_file: str, doc_id: Optional[str] = None):
    """
    Converts chunks from BreadcrumbChunker to the standardized format and saves to file.
    
    Args:
        input_chunks_file: Path to file containing chunks from BreadcrumbChunker
    ...
    with open(output_file, 'w') as f:
        json.dump({
            ...
            "source_metadata": {
                "doc_id": doc_id,
                "filename": Path(input_chunks_file).name,
                "processed_date": None,
                "chunk_count": len(mapped_chunks)
            }
        }, f, indent=2)
    
    print(f"Successfully mapped {len(mapped_chunks)} chunks to {output_file}")
